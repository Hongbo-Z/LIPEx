...
class pipeline(object):
    ...
    def predict(self, text, batch_size = 32): # batch predict mainly for LIPEx
...
for idx in random_test_documents(num_of_test_documents):
    ...
    sample_data, sample_labels, sample_distances, sample_weights, features2use = explainer.sample_data_and_features(new_test.data[idx], c.predict, num_features=union_num, num_samples=5000)
    ...
    # LIPEx reprediction 
    LIPEx_count_list = rePredict(idx,LIPEx_TopK_words,number_of_features_to_remove, pred_label)
...
print('average_LIPEx =',np.average(np.array(LIPEx_count),axis=0).tolist())
