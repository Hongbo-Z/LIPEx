...
import pandas as pd # added by me for debugging and csv output
import math
...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
        ...
        for label in labels:
            (ret_exp.intercept[label],
             ...
             ret_exp.used_features[label]) = self.base.explain_instance_with_data(
    ...
    def __data_labels_distances_new(self,
                            indexed_string,
                            classifier_fn,
                            num_samples,
                            delta=math.pi/2,
        ...
        doc_size = indexed_string.num_words()
         # threshold = doc_size
        threshold = int(doc_size*(math.sin(delta))**2) # delta in radians
        if threshold == 0:
            threshold = 1
        sample = self.random_state.randint(1, threshold + 1, num_samples - 1)

        # with open('LIPEx/sample.txt', 'w') as sf:
        #     sf.write("\n".join(str(x) for x in sample.tolist()))
        data = np.ones((num_samples, doc_size))
        data[0] = np.ones(doc_size)
        features_range = range(doc_size)
        inverse_data = [indexed_string.raw_string()]
        for i, size in enumerate(sample, start=1):
            inactive = self.random_state.choice(features_range, size,
                                                replace=False)
            # with open('LIPEx/inactive.txt', 'a') as iaf:
            #     iaf.write(";".join(str(x) for x in inactive.tolist()))
            #     iaf.write("\n")
            data[i, inactive] = 0
    ...
    def explain_instance_new(self,
        ...
        data, yss, distances = self.__data_labels_distances_new(
            indexed_string, classifier_fn, num_samples,
            distance_metric=distance_metric)
        
        # cosine_theta = 1-distances/100
        # pd.DataFrame(cosine_theta).to_csv("XAI/LIME/CosineTheta.csv",header=None, index=None)
        ...
        ret_exp.predict_proba = yss[0]
        if top_labels:
            labels = np.argsort(yss[0])[-top_labels:] # index of labels by increasing order
            ret_exp.top_labels = list(labels)
            ret_exp.top_labels.reverse()
        ...
        ret_exp.score,
        ret_exp.local_pred, 
        ret_exp.used_features) = self.base.explain_instance_with_data_new(
            data, yss, distances, labels, num_features,
            model_regressor=model_regressor)
        return ret_exp

    def sample_data_and_features(
            self,
            text_instance,
            classifier_fn,
            num_samples=5000,
        ...
        indexed_string = (IndexedCharacters(
            text_instance, bow=self.bow, mask_string=self.mask_string)
                          ...
                          IndexedString(text_instance, bow=self.bow,
                                        split_expression=self.split_expression,
                                        mask_string=self.mask_string))
        self.domain_mapper = TextDomainMapper(indexed_string)
        ...
        sorted_labels = np.argsort(yss[0])
        self.sorted_labels = sorted_labels
        # prepare used features
        weights = self.base.kernel_fn(distances)
        used_features = self.base.feature_selection_new(data, yss, sorted_labels, weights,num_features,method='auto')
        return data, yss, distances, weights, used_features

    def explain_instance_LIPEx(
            self,
            data,
            yss,
            distances,
        ...
        if new_top_labels:
            new_ret_exp = explanation.Explanation(domain_mapper=self.domain_mapper,
                                          class_names=self.class_names,
                                          random_state=self.random_state)
            new_ret_exp.predict_proba = yss[0]
            new_labels = self.sorted_labels[-new_top_labels:]
            new_ret_exp.top_labels = list(new_labels)
            new_ret_exp.top_labels.reverse()
            (new_ret_exp.intercept,
            new_ret_exp.local_exp,
            new_ret_exp.score,
            new_ret_exp.local_pred,
            new_ret_exp.used_features) = self.base.explain_instance_with_data_new(
                data, yss, distances, new_labels, num_features,
                model_regressor=model_regressor,
                weights=weights,
                used_features=used_features)
        else:
            raise ValueError('new_top_labels not specified!')
        
        return new_ret_exp    
    
    def explain_instance_LIPEx_LIME(
        self,
        data,
        yss,
        distances,
        ...
        if old_top_labels:
            old_ret_exp = explanation.Explanation(domain_mapper=self.domain_mapper,
                                            class_names=self.class_names,
                                            random_state=self.random_state)
            old_ret_exp.predict_proba = yss[0]
            old_labels = self.sorted_labels[-old_top_labels:]
            old_ret_exp.top_labels = list(old_labels)
            old_ret_exp.top_labels.reverse()
            for label in true_label:
                (old_ret_exp.intercept[label],
                old_ret_exp.local_exp[label],
                old_ret_exp.score[label],
                old_ret_exp.local_pred[label],
                old_ret_exp.used_features) = self.base.explain_instance_with_data(
                    data, yss, distances, label, len(used_features),
                    model_regressor=model_regressor,
                    feature_selection=self.feature_selection,
                    weights=weights)
        else:
            raise ValueError('old_top_labels not specified!')
        ...
        return old_ret_exp, new_ret_exp
    ...
    #     if old_top_labels:
    #         old_ret_exp = explanation.Explanation(domain_mapper=domain_mapper,
    #                                     class_names=self.class_names,
    #                                     random_state=self.random_state)
    #         old_ret_exp.predict_proba = yss[0]
    #         old_labels = sorted_labels[-old_top_labels:]
    #         old_ret_exp.top_labels = list(old_labels)
    #         old_ret_exp.top_labels.reverse()
    ...
    #     if new_top_labels:
    #         new_ret_exp = explanation.Explanation(domain_mapper=domain_mapper,
    #                                     class_names=self.class_names,
    #                                     random_state=self.random_state)
    #         new_ret_exp.predict_proba = yss[0]
    #         new_labels = sorted_labels[-new_top_labels:]
    #         new_ret_exp.top_labels = list(new_labels)
    #         new_ret_exp.top_labels.reverse()
    #         (new_ret_exp.intercept,
    #         new_ret_exp.local_exp,
